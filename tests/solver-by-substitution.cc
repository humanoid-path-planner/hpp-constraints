...
void test_quadratic ()
{
  ...
  solver.add (Implicit::create(quad, EqualToZero));
  solver.add (expl);
...
void test_quadratic3 ()
{
  ...
  solver.add (Implicit::create (quad, Equality));
  solver.add (expl1);
  solver.add (expl2);
  solver.add (expl3);
...
BOOST_AUTO_TEST_CASE(functions1)
{
  ...
  solver.add(Implicit::create (AffineFunction::create(h), EqualToZero));
...
BOOST_AUTO_TEST_CASE(functions2)
{
  ...
  ImplicitPtr_t impl
    (Implicit::create (AffineFunction::create (Jh),EqualToZero));
  ImplicitPtr_t c2 (impl);
  c2->comparisonType (ComparisonTypes(Equality));
  solver.add(impl);
  BOOST_CHECK_EQUAL(solver.dimension(), 3);
  ...
  c3->comparisonType (ComparisonTypes(Equality));
  solver.add (expl);
...
BOOST_AUTO_TEST_CASE (rightHandSide)
{
  for (size_type i=0; i<1000; ++i) {
    ...
    ComparisonTypes comp (ComparisonTypes::nTimes(N, Equality));
...
BOOST_AUTO_TEST_CASE (rightHandSideFromConfig)
{
  ...
  ComparisonTypes comp2(Equality<<EqualToZero<<EqualToZero<<3*Equality);
...
BOOST_AUTO_TEST_CASE (merge)
{
  ...
  ComparisonTypes comp1 (6 * Equality);
  comp1 [0] = comp1 [2] = comp1 [4] = EqualToZero;
  ComparisonTypes comp2 (6 * Equality);
